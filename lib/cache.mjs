...
export const foldersWritableKey = 'fw'
export const foldersAppendableKey = 'fa'
...
export async function resetFoldersTree (user) {
  ...
  // Reset the writable cache
  k = `${prefix}:${foldersWritableKey}`
  if (user) {
    k += `.${user}.`
  }
  await Cache.del(k)
...
export async function get (user, key) {
  ...
  const hit = (ret !== null && ret !== undefined)
  Metrics.counterInc(hit ? Const.METRICS_CACHE_HITS : Const.METRICS_CACHE_MISSES)
  return ret
}
