...
export function decryptPersonalKey (ekey, personaltoken) {
  ...
  const hash = crypto.pbkdf2Sync(data.password, data.seed.length ? data.seed : Config.get().master_key, Config.get().crypto.personal_key_pbkdf2_iterations, 32, 'sha256')
...
export function randomAESKey () {
  ...
  const key = crypto.pbkdf2Sync(rnd, randomBytes(16), Config.get().crypto.personal_key_pbkdf2_iterations, 32, 'sha256')
  return key
}
