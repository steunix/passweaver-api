...
export function decryptPersonalKey (ekey, personaltoken) {
  ...
  const decrypted = decipher.update(ekey, 'base64')

  return decrypted
}

/**
 * Extract a personal password and seed from the personal token used in the JWT
...
export function decodePersonalToken (personaltoken) {
  ...
  const data = decrypted.substring(Config.get().personal_seed.length)
  const seed = data.split(':')[0]
