/**
 * Local file KMS module for Passweaver
 *
 * With this KMS, the DEK is crypted with the local filesystem master key
 * @module lib/auth
...
export function encrypt (data, algorithm) {
  ...
  const encDEK = encryptDEK(dek)

  return {
    kmsMode: Const.KMS_TYPE_LOCALFILE,
 ...
 * @returns {string} Base64 encoded DEK, in the form 'iv:key'
 */
...
function encryptDEK (dek) {
  const iv = crypto.randomBytes(12)
  const cipher = crypto.createCipheriv('aes-256-gcm', Config.get().master_key, iv)
